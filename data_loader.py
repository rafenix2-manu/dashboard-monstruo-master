...
def load_po_data(odoo_db=None, odoo_user=None, odoo_password=None):
    ...
    ruta = buscar_archivo_local(["Orden de compra (purchase.order).xlsx", "Orden de compra (purchase.order)_2.xlsx", "purchase.order.xlsx"])
...
def fetch_odoo_live(db, user, password):
    try:
        ...
        # OPTIMIZACIÓN: Solo traer órdenes de compra que no estén canceladas y limitar a las últimas 1500
        po_domain = [('state', '!=', 'cancel')]
        po_ids = models.execute_kw(db, uid, password, 'purchase.order', 'search', [po_domain], {'limit': 1500, 'order': 'id desc'})
...
def load_philips_data():
    ...
    ruta = buscar_archivo_local(["Seguimiento Philips _ MPC - Ordenes de compra.xlsx"])
...
def load_config_data():
    ...
    ruta = buscar_archivo_local(["PENDIENTES _ CONFIGURACIONES 2026.xlsx"])
    if ruta and os.path.exists(ruta):
